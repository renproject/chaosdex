...
import "./DEXReserveInterface.sol";
import "openzeppelin-solidity/contracts/token/ERC20/ERC20.sol";

contract DEX {
    mapping (address=>DEXReserveInterface) public reserves;
    ...
    function registerReserve(address _erc20, DEXReserveInterface _reserve) external {
        require(reserves[_erc20] == DEXReserveInterface(0x0), "token reserve already registered");
    ...
    function trade(address payable _to, address _src, address _dst, uint256 _sendAmount) public payable returns (uint256) {
        uint256 recvAmount;
        if (_src == BaseToken) {
            require(reserves[_dst] != DEXReserveInterface(0x0), "unsupported token");
            recvAmount = reserves[_dst].buy(_to, msg.sender, _sendAmount);
        } else if (_dst == BaseToken) {
            require(reserves[_src] != DEXReserveInterface(0x0), "unsupported token");
            recvAmount = reserves[_src].sell(_to, msg.sender, _sendAmount);
        } else {
            require(reserves[_src] != DEXReserveInterface(0x0) && reserves[_dst] != DEXReserveInterface(0x0), "unsupported token");
    ...
    function addLiquidity(
        address _liquidityProvider,  uint256 _maxBaseToken, address _token, uint256 _amount, uint256 _deadline
        ) external returns (uint256) {
            require(reserves[_token] != DEXReserveInterface(0x0), "unsupported token");
            return reserves[_token].addLiquidity(_liquidityProvider, _maxBaseToken, _amount, _deadline);
    }

    function removeLiquidity(address _liquidityProvider, address _token, uint256 _liquidity) external {
            require(reserves[_token] != DEXReserveInterface(0x0), "unsupported token");
            reserves[_token].removeLiquidity(_liquidityProvider, _liquidity);
    }
